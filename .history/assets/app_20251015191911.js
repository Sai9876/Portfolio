...
function animateSkillBars() {
  ...
  bars.forEach(bar => {
    ...
    if (rect.top < windowHeight - 100 && !bar.classList.contains("active")) {
      ...
      fill.style.transition = "width 1.4s cubic-bezier(.2,.9,.3,1)";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const originalTitle = "Hey, I'm Sai";
  const accent = '<span class="accent">Sai</span>';
  const titleParts = ["Hey, I'm ", accent];
  ...
  let i = 0;
  function typeTitle() {
    if (i < titleParts[0].length) {
      cursor.insertAdjacentText("beforebegin", titleParts[0].charAt(i));
      i++;
      setTimeout(typeTitle, 80);
    } else {
      cursor.insertAdjacentHTML("beforebegin", titleParts[1]);
      cursor.remove();
      // After small delay, animate subtitle
      setTimeout(() => {
        typeText(subtitleEl, subtitleEl.textContent.trim(), 30);
      }, 600);
    }
  }
  typeTitle();
...
function animateSkills() {
  ...
  skillCards.forEach(card => {
    ...
    if (rect.top < windowHeight - 100 && !card.classList.contains("visible")) {
...
function animateSoftSkills() {
  ...
  cards.forEach(card => {
    ...
    if (rect.top < winH - 100) {
      setTimeout(() => card.classList.add('visible'), parseFloat(card.dataset.delay) * 1000);
    }
  });
}
window.addEventListener('scroll', animateSoftSkills);
window.addEventListener('load', animateSoftSkills);

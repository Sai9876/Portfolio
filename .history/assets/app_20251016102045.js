// assets/app.js — matched to your index.html + style.css
...
// PROJECT CARD TILT (matches .project-card in CSS/HTML)
...
// REVEAL on scroll (simple, works with .reveal)
...
// PARALLAX (elements with .parallax + data-speed exist in HTML)
...
// CURSOR GLOW + PARTICLES (matches style.css #cursor-glow + .cursor-dot)
...
// SKILLS: animate progress bars (works with .skill-card[data-percent] and .skill-bar span from CSS)
...
// TYPEWRITER (matches #hero-title and #hero-subtitle)
...
// ABOUT & SOFT SKILLS reveal helpers (selectors match CSS)
...
// MOBILE NAV toggle (injects .mobile-only button — CSS supports .mobile-only)
...
// CONTACT form optional handling (only if exists)
...
// Projects / courses / certs quick reveals (selectors match HTML/CSS)
...
function animateCourses() {
  const items = document.querySelectorAll('.course-list li, .cert');
  const winH = window.innerHeight;
  items.forEach(el => {
...
window.addEventListener('load', () => { animateProjects(); animateCourses(); });

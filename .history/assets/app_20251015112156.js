...
// throttle cursor glow (replace your current mousemove block)
let lastGlow = 0;
const glowInterval = 35; // ms - tune between 25-60 for smoothness/perf
...
document.addEventListener('mousemove', (e) => {
  ...
  document.body.appendChild(glow);
  setTimeout(() => glow.remove(), 700); // matches CSS fade time
});

window.addEventListener("scroll", () => {
  const scrollY = window.scrollY;
  gsap.to(".hero-photo img", { y: scrollY * 0.1, duration: 0.3, ease: "power1.out" });
});
document.querySelectorAll('.project-card').forEach(card => {
  card.addEventListener('mouseenter', () => {
    gsap.to(card, { boxShadow: "0 0 20px var(--accent-2)", scale: 1.03, duration: 0.4 });
  });
  card.addEventListener('mouseleave', () => {
    gsap.to(card, { boxShadow: "0 0 0 var(--accent-2)", scale: 1, duration: 0.4 });
  });
});

window.addEventListener('load', () => {
  const vid = document.getElementById('bg-video');
  if (vid) vid.play().catch(()=>{}); // prevents autoplay error in some browsers
});
for (let i = 0; i < 20; i++) {
  ...
  gsap.to(orb, { x: "+=" + (Math.random() * 100 - 50), y: "+=" + (Math.random() * 100 - 50), repeat: -1, yoyo: true, duration: 6 + Math.random() * 4, ease: "sine.inOut" });
}
